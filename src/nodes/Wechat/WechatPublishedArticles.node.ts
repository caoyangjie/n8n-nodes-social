...
export class WechatPublishedArticles implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Wechat Published Articles',
    name: 'wechatPublishedArticles',
    icon: 'file:../../icon/wechat.svg',
    group: ['transform'],
    version: 1,
    description: '获取微信公众号已发布文章列表',
    defaults: {
      name: 'WeChat Published Articles',
    },
    ...
    properties: [
      ...
      {
        displayName: '是否返回详细内容',
        name: 'no_content',
        type: 'boolean',
        default: true,
        required: false,
        description: '是否返回文章详细内容',
      },
    ],
  };
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        const no_content = this.getNodeParameter('no_content', i, true) as boolean;
